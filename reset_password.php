			...
			<div class="row visible-on" style="text-align:center;">
				...
				<h3 style="text-align:center;">Reset Password</h3>
					<form action = "#" method = "POST">
						...
						<input type="password" name="newPass1"></input>
						<br>
						Confirm New Password<br>
						<input type="password" name="newPass2"></input>
						<br>
						...
						if(isset($_POST['reset'])){
							...
							if($password1 !== $password2){
								...
								if(empty($_GET['id']) && empty($_GET['token'])){
									...
									$user = rtrim(base64_decode($_GET['id']));
									@$checkUser = $db->prepare("SELECT username FROM users where username='$user'");
										if($checkUser){
											if(@$db->prepare( "UPDATE users SET password='$pass' where username='$user'")){
												$pass = password_hash($pass, PASSWORD_BCRYPT);
												$res = $db->query( "UPDATE users SET password='$pass' where username='$user'");
